...
write_rrbs_jobs_args() # <1 for single end or 2 for piared end> <raw_data_dir>
{
  for sample_name in $(ls $2); do
    if [[ $1 == 1 ]]; then
      echo $sample_name, -output_dir $sample_name -single_end -input_fastq_file $(realpath $2/$sample_name/*) >> rrbs_jobs.args
    else
      echo $sample_name, -output_dir $sample_name -paired-end -paired_input_fastq_files $(realpath $2/$sample_name/*) >> rrbs_jobs.args
    fi
  done
}
...
cores=(4 1 10 1 1)
rams=(300MB 300MB 16GB 10GB 500MB)
