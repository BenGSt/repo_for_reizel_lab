...
################
# import arbs
setwd("~/OneDriveTechnion/ARBS_meth_prostate_cancer/")
narbs <- import("./arbs_bed_files_from_author/N_ARBS_hg19.bed")
tarbs <- import("./arbs_bed_files_from_author/T_ARBS_hg19.bed")

# debug: use 10 regions for testing
narbs <- narbs[1:100]
tarbs <- tarbs[1:100]
...
# time the main function with 1 core and 10 cores
time <- system.time({
  result <- main(narbs, tarbs, sample, bp_to_pad, tile_width, tile_step, region_min_cov, 1)
})
cat(sprintf("1 core: %f\n", time["elapsed"]))

time_mc <- system.time({
  result <- main(narbs, tarbs, sample, bp_to_pad, tile_width, tile_step, region_min_cov, 4)
})
time_mc2 <- system.time({
  result <- main(narbs, tarbs, sample, bp_to_pad, tile_width, tile_step, region_min_cov, 2)
})

time_mc10_presched <- system.time({
  result <- main(narbs, tarbs, sample, bp_to_pad, tile_width, tile_step, region_min_cov, 10)
})
# time_mc10_no_presched <- system.time({
#   result <- main(narbs, tarbs, sample, bp_to_pad, tile_width, tile_step, region_min_cov, 10)
# })

# print times
cat(sprintf("1 core: %f\n", time["elapsed"]))
cat(sprintf("4 cores: %f\n", time_mc["elapsed"]))
cat(sprintf("2 cores: %f\n", time_mc2["elapsed"]))
cat(sprintf("10 cores prescheduled: %f\n", time_mc10_presched["elapsed"]))
# cat(sprintf("10 cores no preschedualing: %f\n", time_mc10_no_presched["elapsed"]))


# plot for testing
plot(result$narbs$dist_from_center, result$narbs$avg_meth_by_pos,
  type = "l", col = "darkorange1", lwd = 3,
  xlab = "Distance From Center of ARBS [bp]", ylab = "Methylation [%]",
  main = "Test",
  ylim = c(0, 85)
)
lines(result$tarbs$dist_from_center, result$tarbs$avg_meth_by_pos,
