...
trim_illumina_adapter_paired_end() #<R1> <R2>
{
	...
	echo \###################$SCRIPT_NAME \($(date)\)#############
	echo runnig: trim_galore --paired --adapter AGATCGGAAGAGC --adapter2 AAATCAAAAAAAC R1.FQ R2.FQ --cores $N_CORES --fastqc \($(date)\)
	${TRIM_GALORE} --paired --adapter AGATCGGAAGAGC --adapter2 AAATCAAAAAAAC R1.FQ R2.FQ --cores $N_CORES --fastqc
	echo \########################################################
}
...
trim_diversity_adaptors()
{
	...
	if [[ $READ_TYPE == "single_end" ]] ; then
	  ...
	  TRIM_GALORE_OUTPUT_1=$(echo $INPUT_INPUT_FASTQ_1 |awk -F / '{print $NF}'| sed 's/\(\.fastq\|.fq\)\.gz/_trimmed.fq.gz/')
	  TRIM_GALORE_OUTPUT_2=$(echo $INPUT_INPUT_FASTQ_2 |awk -F / '{print $NF}'| sed 's/\(\.fastq\|.fq\)\.gz/_trimmed.fq.gz/')
